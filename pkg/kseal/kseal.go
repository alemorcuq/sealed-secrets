...
func FetchCert(ctx context.Context, clientConfig clientcmd.ClientConfig, certURL string, controllerNs string, controllerName string) (io.ReadCloser, error) {
	if certURL != "" {
...
func FetchPublicKey(r io.Reader) (*rsa.PublicKey, error) {
	data, err := io.ReadAll(r)
	if err != nil {
		return nil, err
	}
...
func Seal(outputFormat string, solveNamespace NamespaceFn, in io.Reader, out io.Writer, codecs runtimeserializer.CodecFactory, pubKey *rsa.PublicKey, scope ssv1alpha1.SealingScope, allowEmptyData bool, overrideName, overrideNamespace string) error {
	...
	return sealedSecretOutput(out, outputFormat, codecs, ssecret)
}

func ValidateSealedSecret(ctx context.Context, clientConfig clientcmd.ClientConfig, controllerNs string, controllerName string, in io.Reader) error {
	conf, err := clientConfig.ClientConfig()
	if err != nil {
		return err
	}
...
func ReEncryptSealedSecret(ctx context.Context, clientConfig clientcmd.ClientConfig, controllerNs string, controllerName string, outputFormat string, in io.Reader, out io.Writer, codecs runtimeserializer.CodecFactory) error {
	conf, err := clientConfig.ClientConfig()
	if err != nil {
		return err
	}
...
func SealMergingInto(outputFormat string, solveNamespace NamespaceFn, in io.Reader, filename string, codecs runtimeserializer.CodecFactory, pubKey *rsa.PublicKey, scope ssv1alpha1.SealingScope, allowEmptyData bool) error {
	// #nosec G304 -- should open user provided file
	...
	if err := Seal(outputFormat, solveNamespace, in, &buf, codecs, pubKey, scope, allowEmptyData, orig.Name, orig.Namespace); err != nil {
		return err
	}
...
func UnsealSealedSecret(privateKeys []string, outputFormat string, w io.Writer, in io.Reader, codecs runtimeserializer.CodecFactory) error {

	privKeys, err := readPrivKeys(privateKeys)
	if err != nil {
		return err
	}
