...
func run(w io.Writer, fs *flag.FlagSet, gofs *goflag.FlagSet, args []string) error {
	var flags Flags
...
func Run(w io.Writer, cfg *Config) (err error) {
	...
	if flags.Unseal {
		return kubeseal.UnsealSealedSecret(flags.PrivKeys, flags.OutputFormat, w, input, scheme.Codecs)
	}
	...
	f, err := kubeseal.FetchCert(cfg.ctx, cfg.clientConfig, flags.CertURL, flags.ControllerNs, flags.ControllerName)
	if err != nil {
		return err
	}
	defer f.Close()
	...
	pubKey, err := kubeseal.FetchPublicKey(f)
	if err != nil {
		return err
	}

	if flags.MergeInto != "" {
		return kubeseal.SealMergingInto(flags.OutputFormat, cfg.solveNamespace, input, flags.MergeInto, scheme.Codecs, pubKey, flags.SealingScope, flags.AllowEmptyData)
	}
	...
	return kubeseal.Seal(flags.OutputFormat, cfg.solveNamespace, input, w, scheme.Codecs, pubKey, flags.SealingScope, flags.AllowEmptyData, flags.SecretName, "")
}
...
func main() {
	if err := run(os.Stdout, flag.CommandLine, goflag.CommandLine, os.Args); err != nil {
