...
import (
	...
	kubeseal "github.com/bitnami-labs/sealed-secrets/pkg/kseal"
)
...
func TestParseKey(t *testing.T) {
	key, err := FetchPublicKey(strings.NewReader(testCert))
	if err != nil {
...
func TestOpenCertFile(t *testing.T) {
	...
	for _, certURL := range testCases {
		f, err := kubeseal.FetchCert(testConfig(&flags), certURL)
		if err != nil {
...
func TestSeal(t *testing.T) {
	key, err := kubeseal.FetchPublicKey(strings.NewReader(testCert))
	if err != nil {
		t.Fatalf("Failed to parse test key: %v", err)
	}
...
func TestUnseal(t *testing.T) {
	...
	if err := UnsealSealedSecret(privateKeys, outputFormat, &buf, bytes.NewBuffer(ss), scheme.Codecs); err != nil {
		t.Fatal(err)
	}

	secret, err := kubeseal.ReadSecret(scheme.Codecs.UniversalDecoder(), &buf)
	if err != nil {
		t.Fatal(err)
	}
...
func TestUnsealList(t *testing.T) {
	...
	prettyEnc, err := kubeseal.PrettyEncoder(scheme.Codecs, runtime.ContentTypeJSON, v1.SchemeGroupVersion)
	if err != nil {
		t.Fatal(err)
	}
	...
	var buf bytes.Buffer
	flags := Flags{PrivKeys: []string{pkFile.Name()}}
	if err := kubeseal.UnsealSealedSecret(&flags, &buf, bytes.NewBuffer(ss), scheme.Codecs); err != nil {
		t.Fatal(err)
	}
...
func TestMergeInto(t *testing.T) {
	...
	merge := func(t *testing.T, newSecret, oldSealedSecret []byte) *ssv1alpha1.SealedSecret {
		...
		if err := kubeseal.SealMergingInto(testConfig(&flags), buf, f.Name(), scheme.Codecs, pubKey, ssv1alpha1.DefaultScope, false); err != nil {
			t.Fatal(err)
		}
	...
	t.Run("updated", func(t *testing.T) {
		...
		orig, err := kubeseal.DecodeSealedSecret(scheme.Codecs, origSrc)
		if err != nil {
			t.Fatal(err)
		}
...
func TestVersion(t *testing.T) {
	...
	err := run(buf, testVersionFlags, nopFlags, []string{"--version"})
	if err != nil {
		t.Fatal(err)
	}
